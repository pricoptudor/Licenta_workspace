...
class BaseTrainer(metaclass=abc.ABCMeta):
    ...
    def __init__(self, dataset_manager, logdir, logging_period, training_ops=None,
        ...
        if self._ops.training_placeholder is None:
            self._ops.training_placeholder = tf.Variable(False, dtype=tf.bool, name='is_training')

        if write_summaries:
            self._writer = tf.summary.create_file_writer(logdir=self._logdir)
    ...
    def training_step(self, dataset_name, feed_dict=None, write_summaries=True, log=True):
        ...
        if self._step % self._logging_period == 0:
            ...
            if log:
                print('step: {}, loss: {}'.format(self._step, train_loss))
    ...
    def load_variables(self, checkpoint_name, checkpoint_file=None):
        ...
        print('Variables restored from {}'.format(checkpoint_file))
        self._step += 1

    def write_scalar_summary(self, name, value, step=None):
        with self._writer.as_default():
            tf.summary.scalar(name, value, step if step is not None else self._step)
...
class BasicTrainer(BaseTrainer):
    ...
    def __init__(self, dataset_manager, logdir, logging_period, validation_period=None,
        ...
        self._savers = {
            'latest': tf.train.Checkpoint(name='latest', max_to_keep=1),
            'best': tf.train.Checkpoint(name='best', max_to_keep=1)
        }
...
def training_validation_loop(training_step_fn, init_fn=None, validation_fn=None,
    ...
    while True:
        ...
        try:
            ...
            print(f'Training finished after {state.step} steps, loss: {state.train_loss}')
            break
