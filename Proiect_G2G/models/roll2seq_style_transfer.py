...
from muse.Components import EmbeddingLayer, RNNDecoder, RNNLayer, InputWrapper, CNN
from muse.Trainer import BasicTrainer
from muse.Utils import DatasetManager, create_train_op, make_simple_dataset, prepare_train_and_val_data, set_random_seed, \
                        filter_sequence, set_note_fields
from muse.Encodings import PianoRollEncoding, PerformanceEncoding

from note_seq.protobuf import music_pb2

from g2g_io import EvalPipeline, MidiPipeline, TrainLoader
...
class Model:
    ...
    def __init__(self, dataset_manager, train_mode, vocabulary, sampling_seed=None):
        ...
        self.softmax_temperature = tf.Variable([], dtype=tf.float32, name='softmax_temperature')
        self._is_training = tf.Variable(False, dtype=tf.bool, name='is_training')

        self.dataset_manager = dataset_manager

        inputs, style_inputs, decoder_inputs, decoder_targets = self.dataset_manager.get_next()

        cnn = CNN(training=self._is_training,
                  name='encoder_cnn')
        rnn = RNNLayer(training=self._is_training,
                       name='encoder_rnn')

        encoder_states, _ = rnn(cnn(inputs))

        embeddings = EmbeddingLayer(input_size=len(vocabulary),
                                    output_size=300,
                                    name='embedding_layer')

        style_cnn = CNN(training=self._is_training,
                        name='style_encoder_cnn')
        style_rnn = RNNLayer(training=self._is_training,
                             name='style_encoder_rnn')
        
        style_projection = Dense(name='style_projection')
        ...
        attention = BahdanauAttention(units=300, memory=encoder_states, name='attention_mechanism')

        self.decoder = RNNDecoder(max_length = 2000,
    ...
    def _make_train_ops(self):
        lr_decay = ExponentialDecay(initial_learning_rate=0.001,
                                    decay_steps=3000,
                                    decay_rate=0.5)
        train_op = create_train_op(loss=self.loss, 
...
class Experiment:
    ...
    def __init__(self, logdir, train_mode, sampling_seed=None):
        ...
        self.input_encoding = PianoRollEncoding(sampling_frequency=4, normalize=True)
        self.output_encoding = PerformanceEncoding(time_unit=1/12,
                                                   velocity_unit=16,
                                                   use_velocity=True,
        ...
        self.model = Model(dataset_manager=self.dataset_manager,
        ...
        self.trainer = BasicTrainer(dataset_manager=self.dataset_manager,
        ...
        if train_mode:
            ...
            train_loader = TrainLoader(random_seed=random_seed,
            ...
            val_loader = TrainLoader(random_seed=random_seed,
                                     reseed=True,
    ...
    def run(self, pipeline, batch_size=None, filters='program', sample=False,
        ...
        for seq, meta in zip(sequences, metadata_list):
            ...
            if meta['note_features'] is not None:
                # The output has velocity information, do not override it
    ...
    def _load_data(self, loader, training=False, encode=True, apply_filters=True,
        ...
        def generator():
            ...
            if training:
                print(f'Done loading data: {i} examples; '
    ...
    def _normalize_velocity(self, seq):
        ...
        target_mean = 64.25399574283072
        target_std = np.sqrt(432.9165195560093)
